...
#define HUDCOLOR [UIColor hexStringToColor:@"2dc07a"]
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.labelColor = HUDCOLOR;
		self.detailsLabelFont = [UIFont boldSystemFontOfSize:kDetailsLabelFontSize];
		self.detailsLabelColor = HUDCOLOR;
		self.activityIndicatorColor = HUDCOLOR;
		self.xOffset = 0.0f;
		self.yOffset = 0.0f;
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        if (!isActivityIndicator) {
            ...
            self.indicator.tintColor = HUDCOLOR;
            self.indicator.center = CGPointMake(CGRectGetMidX(effectView.bounds), CGRectGetMidY(effectView.bounds));
...
- (void)layoutSubviews {
    ...
    CGRect boxRect = CGRectMake(round((allRect.size.width - size.width) / 2) + self.xOffset,round((allRect.size.height - size.height) / 2) + self.yOffset, size.width, size.height);
    effectView.frame = boxRect;
